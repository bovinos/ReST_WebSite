    ...
    private void action_submit_ProfileUserSignUpData(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        User user;
            ...
            if ((request.getParameter("username") != null && request.getParameter("username").length() > 0) 
                    && !(request.getParameter("username").equals(user.getUsername()))) { user.setUsername(request.getParameter("username")); }
             if ((request.getParameter("mail") != null && request.getParameter("mail").length() > 0)
                    && !(request.getParameter("mail").equals(user.getMail()))) { user.setMail(request.getParameter("mail"));}
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        switch(section){
            case 1: //Siamo nel ramo "Modifica dati di registrazione"...
                if (request.getParameter("ModificaDati")!= null){ //Abbiamo inviato i dati modificati...
        try {
